...
// Tasks:
// Harrison - Budget endpoints and FE (if any FE)
// Cullen - Endpoints for Expenses
// Skyler - FE Home Page (and other pages)
...
const ExpenseSchema = new mongoose.Schema({
  description: String,
  day: Number, // Check for what its called
  amount: Double,
  vendor: String,
  categoryID: String,
});
...
app.get('/budget/read_all', (req, res) => {
    try {
      let items = await BudgetItem.find();
      res.send(items);
    } catch (error) {
      console.log(error);
      res.sendStatus(500);
